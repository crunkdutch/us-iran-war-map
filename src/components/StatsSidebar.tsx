...
export default function StatsSidebar({ attacks, sitreps, visible }: Props) {
  ...
  const stats = useMemo(() => {
    ...
    // Hormuz data inlined to avoid Turbopack import issues on Vercel
    const hormuzData: { date: string; daily: number; label: string; note: string }[] = [
    ...
    const peakHormuz = Math.max(...hormuzData.map(d => d.daily))
